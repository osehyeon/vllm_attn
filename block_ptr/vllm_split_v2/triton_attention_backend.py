"""vLLM v1 AttentionBackend — Triton fused attention with split-KV decode (block_ptr variant).

This is the **block_ptr/vllm_split_v2** variant: same algorithm as ptr/vllm_split_v2
(prefill kernel + 3 decode kernels with FlashDecoding-style split-KV), but all
Q/K/V/O loads/stores use `tl.make_block_ptr`. See triton_attn.py and NOTES.md.
...
class MyTritonImpl(AttentionImpl[MyTritonMetadata]):
    ...
    def forward(
        ...
        if q_len == s_len:
            ...
            q = query[:N].view(1, N, Hq, D).transpose(1, 2).contiguous()
            k = key[:N].view(1, N, Hkv, D).transpose(1, 2).contiguous()
            v = value[:N].view(1, N, Hkv, D).transpose(1, 2).contiguous()
            o = triton_attention_prefill(q, k, v, scale=self.scale)
            output[:N].copy_(o.transpose(1, 2).reshape(N, Hq, D))
            FIRE_COUNTER["prefill"] += 1
            if _LOG_FIRE and FIRE_COUNTER["prefill"] <= _LOG_EVERY_N:
                _logger.warning("MyTritonImpl[v2/block_ptr].forward fired (prefill) tokens=%d", N)
        else:
            # === decode === (q_len=1, split-KV over full KV context)
            q = query[:1].view(1, 1, Hq, D).transpose(1, 2).contiguous()
            k_full, v_full = _gather_kv(
                key_cache, value_cache,
                attn_metadata.block_table[0], s_len,
            )
            o = triton_attention_decode(q, k_full, v_full, scale=self.scale)
            output[:1].copy_(o.reshape(1, Hq, D))
            FIRE_COUNTER["decode"] += 1
            if _LOG_FIRE and FIRE_COUNTER["decode"] <= _LOG_EVERY_N:
                _logger.warning(
                    "MyTritonImpl[v2/block_ptr].forward fired (decode, split-KV) s_len=%d", s_len,
                )
...
def register() -> None:
    """Register MyTritonBackend (split-KV v2) into the CUSTOM slot.
...
if __name__ == "__main__":
    ...
    assert MyTritonBackend.get_supported_head_sizes() == []
