...
block_ptr variant: Q/O use block_ptr (Snippets D/E/F). K/V use fresh
tl.make_block_ptr per iteration (Snippet B) — tl.advance is not possible
because the physical block address changes each step via block_table lookup.
BLOCK_N == BLOCK_SIZE is enforced so each iteration covers exactly one
physical block, making the fresh block_ptr idiom correct.
"""
...
def _fwd_kernel_prefill_paged(
    ...
    BLOCK_N: tl.constexpr,   # == BLOCK_SIZE enforced by wrapper
    ...
    # Load Q tile via block_ptr (Snippet D)
    ...
    # Iterate over KV tiles: one physical block per iteration (BLOCK_N == BLOCK_SIZE)
    for n in range(0, tl.cdiv(S, BLOCK_N)):
        n_start = n * BLOCK_N
        offs_n = n_start + tl.arange(0, BLOCK_N)
        kv_mask = offs_n < S

        # Paged lookup: one scalar block index per iteration (Snippet B)
        ...
        # K transposed view (BLOCK_D, BLOCK_SIZE) via order=(0,1) — fresh each iter
...
def _fwd_kernel_decode_paged(
    ...
    # Load single query token via 1-D block_ptr (Snippet E)
    Q_block_ptr = tl.make_block_ptr(
        base=Q + pid_bh * stride_qb,
        shape=(BLOCK_D,),
        strides=(stride_qd,),
    ...
    for n in range(0, tl.cdiv(S, BLOCK_N)):
        ...
        # Paged lookup: one physical block per iteration
        physical_block_idx = tl.load(block_table + batch_idx * stride_bt_seq + n)
        kv_block_base = (
            physical_block_idx * stride_cache_block
            + kv_head_idx * stride_cache_head
        ...
        # K [BLOCK_SIZE, BLOCK_D] — no transpose, use element-wise reduction
        K_block_ptr = tl.make_block_ptr(
            base=K_cache + kv_block_base,
            shape=(BLOCK_SIZE, BLOCK_D),
            strides=(stride_cache_slot, stride_cache_d),
