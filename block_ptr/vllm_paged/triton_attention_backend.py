...
class MyTritonImpl(AttentionImpl[MyTritonMetadata]):
    ...
    def forward(
        ...
        # Paged metadata — passed straight to the kernels
        seq_lens_t = attn_metadata.seq_lens          # [B] int32
        block_table_t = attn_metadata.block_table    # [B, max_blocks] int32

        if q_len == s_len:
            # === prefill === (q_len == kv_len, causal) — paged KV read inside kernel
            q = query[:N].view(1, N, Hq, D).transpose(1, 2).contiguous()    # [1, Hq, N, D]
            o = triton_attention_prefill_paged(
                q, key_cache, value_cache, block_table_t, seq_lens_t,
                scale=self.scale,
            )                                                                # [1, Hq, N, D]
            output[:N].copy_(o.transpose(1, 2).reshape(N, Hq, D))
            FIRE_COUNTER["prefill"] += 1
            if _LOG_FIRE and FIRE_COUNTER["prefill"] <= _LOG_EVERY_N:
                _logger.warning("MyTritonImpl.forward fired (prefill paged) tokens=%d", N)
        else:
            # === decode === (q_len=1) — paged KV read inside kernel
            q = query[:1].view(1, 1, Hq, D).transpose(1, 2).contiguous()    # [1, Hq, 1, D]
            o = triton_attention_decode_paged(
                q, key_cache, value_cache, block_table_t, seq_lens_t,
                scale=self.scale,
            )                                                                # [1, Hq, 1, D]
            ...
            if _LOG_FIRE and FIRE_COUNTER["decode"] <= _LOG_EVERY_N:
                _logger.warning("MyTritonImpl.forward fired (decode paged) s_len=%d", s_len)
