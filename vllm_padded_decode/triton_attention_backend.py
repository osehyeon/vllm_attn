...
class MyTritonImpl(AttentionImpl[MyTritonMetadata]):
    ...
    def forward(
        ...
        from triton_attn import triton_attention  # local import avoids circular dep
        ...
        if q_len == s_len:
            ...
            o = triton_attention(q, k, v, scale=self.scale)                  # [1, Hq, N, D]
            ...
            # === decode (q_len=1, s_len>1) — reuse prefill kernel via q zero-pad ===
            # No dedicated decode kernel here: we pad q to kv_len so that the
            # prefill kernel's q_len==kv_len assumption holds. Causal mask then
            # makes the last q position attend all past keys (correct decode);
            # the first s_len-1 padded rows compute garbage which we discard.
            # Cost: O(s_len^2) — intentional simplicity over performance.
            q = query[:1].view(1, 1, Hq, D).transpose(1, 2).contiguous()    # [1, Hq, 1, D]
            k_full, v_full = _gather_kv(
                key_cache, value_cache,
                attn_metadata.block_table[0], s_len,
            )                                                                # [1, Hkv, s_len, D]
            q_pad = torch.zeros(1, Hq, s_len, D, dtype=q.dtype, device=q.device)
            q_pad[:, :, -1, :] = q[:, :, 0, :]
            o_pad = triton_attention(q_pad, k_full, v_full, scale=self.scale)  # [1, Hq, s_len, D]
            output[:1].copy_(o_pad[:, :, -1, :].reshape(1, Hq, D))
