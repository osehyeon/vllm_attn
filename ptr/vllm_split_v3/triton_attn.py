...
v3: prefill kernel generalized to **chunked prefill** — accepts q_len <= kv_len.
The Q tokens are interpreted as the LAST q_len tokens of a sequence whose full
length is kv_len, with absolute-position causal mask:

    q_abs = kv_len - q_len + offs_m         # [BLOCK_M]
    causal: q_abs[:, None] >= offs_n[None, :]

Three special cases collapse into the same kernel:
    q_len == kv_len     → pure prefill      (q_abs = [0, kv_len))
    1 < q_len < kv_len  → chunked prefill   (q_abs = [kv_len - q_len, kv_len))
    q_len == 1          → still works, but the dedicated decode kernel is faster

Naming follows vLLM conventions (see vllm/v1/attention/ops/triton_{prefill,decode}_attention.py):
    _fwd_kernel_prefill   : q_len <= kv_len, causal mask in absolute positions
...
# ---------------------------------------------------------------------------
# Prefill kernel — q_len <= kv_len, causal in absolute positions.
# Handles both pure prefill (q_len == kv_len) and chunked prefill (q_len < kv_len).
# ---------------------------------------------------------------------------
...
def _fwd_kernel_prefill(
    ...
    sm_scale,
    Q_LEN,                            # length of the Q chunk
    KV_LEN,                           # full accumulated context length
    ...
    # Absolute query positions within the full sequence of length KV_LEN.
    # Q is interpreted as the LAST Q_LEN tokens of that sequence.
    #   pure prefill (Q_LEN == KV_LEN):   q_abs = offs_m
    #   chunked     (1 < Q_LEN < KV_LEN): q_abs = [KV_LEN - Q_LEN, ..., KV_LEN - 1]
    q_abs = KV_LEN - Q_LEN + offs_m
    ...
    for n in range(0, tl.cdiv(KV_LEN, BLOCK_N)):
        ...
        # Causal in absolute positions — works for pure and chunked prefill alike
...
def _fwd_kernel_decode(
    ...
    # Load the single query token: [BLOCK_D]
    q = tl.load(Q + pid_bh * stride_qb + offs_d * stride_qd)
    q = q.to(tl.float32)
    ...
    for n in range(0, tl.cdiv(S, BLOCK_N)):
        ...
        # K tile [BLOCK_N, BLOCK_D] — kept in input dtype to save smem,
        # promoted to fp32 at use-site in registers.
        k = tl.load(
            K + pid_bh * stride_kb
        ...
        # V tile [BLOCK_N, BLOCK_D] — same smem-saving pattern
        v = tl.load(
            V + pid_bh * stride_vb
...
def triton_attention_prefill(
    ...
    """
    Causal attention with chunked-prefill support.

    The Q tokens are interpreted as the LAST q_len tokens of a sequence whose
    full length is kv_len. Causal mask uses absolute positions, so this single
    function handles both pure prefill and chunked prefill:

        q_len == kv_len   → pure prefill    (q_abs = [0, kv_len))
        q_len <  kv_len   → chunked prefill (q_abs = [kv_len - q_len, kv_len))

    q: [B, Hq,  q_len,  D]   fp16 or bf16
    k: [B, Hkv, kv_len, D]   same dtype, kv_len >= q_len
    v: [B, Hkv, kv_len, D]   same dtype
    returns o: [B, Hq, q_len, D]
    """
    ...
    assert v.shape[2] == kv_len, f"k/v kv_len mismatch: {kv_len} vs {v.shape[2]}"
    ...
    q_3d = q.reshape(B * Hq, q_len, D)
    k_3d = k.reshape(B * Hq, kv_len, D)
    v_3d = v.reshape(B * Hq, kv_len, D)
    o_3d = torch.empty_like(q_3d)

    BLOCK_D = D
    BLOCK = _cap_block_for_head_dim(_get_block_size(q.dtype), D)
    grid = (B * Hq, triton.cdiv(q_len, BLOCK))
    ...
    return o_3d.reshape(B, Hq, q_len, D)


# Explicit alias for callers that want to spell out the chunked intent.
triton_attention_chunked_prefill = triton_attention_prefill
...
if __name__ == "__main__":
    ...
    def _check_prefill(dtype, D, atol, label):
        ...
        ok = err < atol
        print(f"  prefill   {label:24s} max_abs_err = {err:.6f}  →  {'PASS' if ok else 'FAIL'}")
        return ok

    def _check_chunked(dtype, D, q_len, kv_len, atol, label):
        """Q is the LAST q_len tokens of a sequence of length kv_len.
        Reference: full causal SDPA, then take last q_len rows."""
        B, Hq, Hkv = 1, 16, 8
        assert q_len <= kv_len
        ...
        ref_chunk = ref_full[:, :, kv_len - q_len:, :]                  # [B, Hq, q_len, D]

        q_chunk = q_full[:, :, kv_len - q_len:, :].contiguous()
        ours = triton_attention_prefill(q_chunk, k, v)
        err = (ours.float() - ref_chunk.float()).abs().max().item()
        ok = err < atol
        print(f"  chunked   {label:24s} max_abs_err = {err:.6f}  →  {'PASS' if ok else 'FAIL'}")
        return ok
    ...
    def _check_decode(dtype, kv_len, atol, label):
        ...
        ok = err < atol
        print(f"  decode    {label:24s} max_abs_err = {err:.6f}  →  {'PASS' if ok else 'FAIL'}")
        return ok
    ...
    print("chunked prefill (1 < q_len < kv_len, causal in absolute positions)")
    for dtype, atol in [(torch.float16, 1e-2), (torch.bfloat16, 3e-2)]:
        for D, q_len, kv_len in [
            (128, 32, 128),
            (128, 16, 128),
            (128, 64, 256),
            (64,  31, 97),     # non-multiple-of-BLOCK
            (256,  8, 64),
        ]:
            results.append(
                _check_chunked(
                    dtype, D, q_len, kv_len, atol,
                    f"{str(dtype).split('.')[-1]}/q={q_len}/kv={kv_len}/D={D}",
                )
            )
