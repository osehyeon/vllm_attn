...
FIRE_COUNTER: dict[str, int] = {"prefill": 0, "chunked": 0, "decode": 0}
...
class MyTritonImpl(AttentionImpl[MyTritonMetadata]):
    ...
    def forward(
        ...
        if q_len == s_len:
            # === pure prefill === (q_len == kv_len, causal, cold start)
            # Q and KV are both the new tokens — no past context yet, so we
            # can pass key/value directly without going through the paged cache.
            ...
            # Q is the current chunk (q_len new tokens). KV must include the
            # full accumulated context (s_len tokens) — the new chunk's K/V
            # were just written into the paged cache by the call above, so
            # gathering the cache yields prior context + new chunk together.
            q = query[:N].view(1, N, Hq, D).transpose(1, 2).contiguous()    # [1, Hq, q_len, D]
            k_full, v_full = _gather_kv(
                key_cache, value_cache,
                attn_metadata.block_table[0], s_len,
            )                                                                # [1, Hkv, s_len, D]
            o = triton_attention_prefill(q, k_full, v_full, scale=self.scale)  # [1, Hq, q_len, D]
            output[:N].copy_(o.transpose(1, 2).reshape(N, Hq, D))
            FIRE_COUNTER["chunked"] += 1
            if _LOG_FIRE and FIRE_COUNTER["chunked"] <= _LOG_EVERY_N:
                _logger.warning(
                    "MyTritonImpl.forward fired (chunked prefill) q_len=%d s_len=%d",
                    q_len, s_len,
                )
...
if __name__ == "__main__":
    ...
    assert "chunked" in FIRE_COUNTER
