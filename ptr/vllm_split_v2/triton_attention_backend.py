"""vLLM v1 AttentionBackend — Triton fused attention with split-KV decode.

This is the **vllm_split_v2** variant of vllm_split: same prefill kernel, but the
decode path uses FlashDecoding-style split-KV (phase-1 partial + phase-2 reduce
log-sum-exp combine). See triton_attn.py and NOTES.md.
    ...
    def forward(
        ...
        if q_len == s_len:
            ...
            if _LOG_FIRE and FIRE_COUNTER["prefill"] <= _LOG_EVERY_N:
                _logger.warning("MyTritonImpl[v2].forward fired (prefill) tokens=%d", N)
        else:
            ...
            if _LOG_FIRE and FIRE_COUNTER["decode"] <= _LOG_EVERY_N:
                _logger.warning(
                    "MyTritonImpl[v2].forward fired (decode, split-KV) s_len=%d", s_len,
                )
